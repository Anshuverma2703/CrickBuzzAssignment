...
if(getvalue==1){
    ...
    function searching(res){
       ...
       for(let i =0;i<element.length;i++){
            if(element[i].innerText === res[j] && element[i].style.backgroundColor!= "yellow"){
                // console.log(res[j]);
                // let temp = i;
    ...
    function horizontaldirectionSearch(i,j,res){
        let flag = true
        let k;
        if(element[i+1].innerText === res[++j]&& element[i+1].style.backgroundColor != "yellow" ){// to change the row
            for(k = 1;k<res.length;k++){
                console.log("akjbk")
